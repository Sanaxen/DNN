...
int main(){
    ...
    for(int k=0; k<epochNums; k++){
        ...
        float loss_mean = sum_loss/((float)totalSampleSize/ (float)batchSize);
        float accurecy_mean = accurecy/((float)totalSampleSize/ (float)batchSize);
        cout << "epoch:" << k+1 << " loss:" << loss_mean << " accurecy:" << setprecision(3) << accurecy_mean*100.0 << "% time:" << elapsed << "ms" << endl;
    }
