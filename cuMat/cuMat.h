...
public:
    ...
    cuMat() {
        rows = 0;
        cols = 0;
		cublasStatus_t stat = cublasCreate(&cudaHandle);
    ...
    cuMat(const cuMat &a) {
        ...
        if (error != cudaSuccess)
            printf("cuMat copy constractor cudaMemcpy error:%d\n", error);
    ...
    void memMallocDevice() {
        ...
        if (error != cudaSuccess) printf("cudaMemcpy error:%d\n", error);
    ...
    void new_matrix(int rows, int cols) {
        ...
        if (this->rows != rows || this->cols != cols) {
            ...
            error = cudaMalloc((void**) &mDevice,
                    rows * cols * sizeof(*mDevice));
            if (error != cudaSuccess) printf("cuMat::new_matrix cudaMalloc error:%d\n", error);
    ...
    void memHostToDevice() {
        ...
        if (error != cudaSuccess) printf("memHostToDevice cudaMemcpy error:%d\n", error);
    }
    void memDeviceToHost() {
        ...
        if (error != cudaSuccess)
            printf("memDeviceToHost cudaMemcpy error:%d\n", error);
    }
    ...
    void memSetHost(float *v) {
        ...
        cudaError_t error = cudaMemcpy(mDevice, v,
                rows * cols * sizeof(*mDevice), cudaMemcpyHostToDevice);
        if (error != cudaSuccess)
            printf("memSetHost cudaMemcpy error:%d\n", error);
    }
    void memSetDevice(float *v) {
        cudaError_t error = cudaMemcpy(mDevice, v,
                                       rows * cols * sizeof(*mDevice), cudaMemcpyDeviceToDevice);
        if (error != cudaSuccess)
            printf("memSetDevice cudaMemcpy error:%d\n", error);
    }
    void memSetDeviceRow(float *v, int row_index) {
        ...
        if (error != cudaSuccess)
            printf("memSetDeviceRow cudaMemcpy error:%d\n", error);
    }
    void memSetDeviceCol(float *v, int col_index) {
        ...
        if (error != cudaSuccess)
            printf("memSetDeviceCol cudaMemcpy error:%d\n", error);
    }
    ...
    cuMat &operator=(const cuMat &a) {
        ...
        cudaError_t error = cudaMemcpy(mDevice, a.mDevice,
                rows * cols * sizeof(*mDevice), cudaMemcpyDeviceToDevice);
        if (error != cudaSuccess)
            printf("cuMat operator= cudaMemcpy error:%d\n", error);
    ...
    void plus(const cuMat &b, cuMat &r) {
        ...
        if (stat != CUBLAS_STATUS_SUCCESS)
            cout << "cannot cublasSgeam:" << stat << endl;
        cudaThreadSynchronize();
    }
    ...
    void dot(const cuMat &b, cuMat &r) {
		...
		cublasStatus_t stat0 = cublasCreate(&cudaHandle);
		checkCublasErrors(stat0);
		...
		if (stat != CUBLAS_STATUS_SUCCESS)
			cout << "cannot cublasSgemm dot:" << stat << endl;
		checkCublasErrors(stat);
        cudaThreadSynchronize();
    }
    void dot_plus(const cuMat &b, cuMat &r) {
        ...
        checkCublasErrors(stat);
                if (stat != CUBLAS_STATUS_SUCCESS)
                    cout << "cannot cublasSgemm dot_plus:" << stat << endl;
                cudaThreadSynchronize();
        }
    ...
    void transpose_dot_plus(const cuMat &b, cuMat &r) {
        ...
        checkCublasErrors(stat);
            if (stat != CUBLAS_STATUS_SUCCESS)
                cout << "cannot cublasSgemm transpose_dot_plus:" << stat << endl;
            cudaThreadSynchronize();
    }
    void dot_transpose_plus(const cuMat &b, cuMat &r) {
        ...
        checkCublasErrors(stat);
            if (stat != CUBLAS_STATUS_SUCCESS)
                cout << "cannot cublasSgemm dot_transpose_plus:" << stat << endl;
            cudaThreadSynchronize();
    }
